...
const { DeliveryPackage, PostPackage } = require("./DeliveryPackage.js");
...
Merchant.prototype.postSalesReceipt = async function (
  ...
  transactionArray,
  callback) {
  transactionArray = transactionArray.map(transaction => {
    return { ...transaction }
  });
...
Merchant.prototype.putSalesReceipt = async function (
  ...
  tansactionArray,
  callback) {
  tansactionArray = tansactionArray.map(transaction => {
    return { ...transaction }
  });
 ...
 */
Merchant.prototype.deleteReceipt = async function (receiptId, callback) {
  let url = `${this.salesPath}/receipts/${receiptId}?licenseNumber=${this.licenseNo}`;
  try {
...
/**
 * Method to create a sales transaction
 * @param { string } date
 * @param { array } transactionArray
 */
Merchant.prototype.createSalesTransaction = async function (date, transactionArray, callback) {
  let url = `${this.salesPath}/transactions/${date}?licenseNumber=${this.licenseNo}`;
  try {
    transactionArray = transactionArray.map(transaction => {
      return { ...transaction }
    });
...
/**
 * Method to put a sales transaction
 * @param { string } date
 * @param { array } transactionArray
 */
Merchant.prototype.putSalesTransaction = async function (date, transactionArray, callback) {
  let url = `${this.salesPath}/transactions/${date}?licenseNumber=${this.licenseNo}`;
  try {
    transactionArray = transactionArray.map(transaction => {
      return { ...transaction }
 ...
 * @param { Function } callback
 */
Merchant.prototype.createPackage = async function (packages, callback) {
  let url = `${this.packagePath}/packages/v1/create?licenseNumber=${this.licenseNo}`;
  try {
    /**Forming object out of the template to send desired o/p to API */
    packages = packages.map(package => {
      return { ...package };
    });
    packages.forEach(package => {
      package = package.Ingredients = package.Ingredients.map(ingredient => {
        return { ...ingredient };
      });
    });
...
/**
 * Method to create a testing package
 * @param { PostPackage } package
 * @param { Function } callback
 */
Merchant.prototype.createTestingPackage = async function (packages, callback) {
  let url = `${this.packagePath}/packages/v1/create/testing?licenseNumber=${this.licenseNo}`;
  try {
    /**Forming object out of the template to send desired o/p to API */
    packages = packages.map(package => {
      return { ...package };
