 ...
 * @param { Function } callback 
 */
Merchant.prototype.postTransfers = function (transfers, callback) {
  try {
    let url = `${this.transferPath}/external/incoming?licenseNumber=${this.licenseNo}`;
    let response = await http.post(url, transfers, {
      username: this.vendorkey,
      password: this.userkey
    });
 ...
 * @param { Function } callback 
 */
Merchant.prototype.putTransfers = function (transfers, callback) {
  try {
    let url = `${this.transferPath}/external/incoming?licenseNumber=${this.licenseNo}`;
    let response = await http.put(url, transfers, {
      username: this.vendorkey,
      password: this.userkey
    });
    callback(response.data);
